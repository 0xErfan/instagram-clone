    ...
    <div class="max-w-[520px] w-full p-4 bg-transparent backdrop-blur-md border rounded-md border-white/50">
      ...
      <form @submit.prevent="submit_login_form" class="flex flex-col gap-4">
        <input :value="loginData.name ?? ''" @change="e => update_form(e, 'name')"
          class="bg-transparent border-b-2 outline-none pb-2 border-red-600" placeholder="Enter your name" type="text">
        ...
        <input :value="loginData.email ?? ''" @change="e => update_form(e, 'email')"
          class="bg-transparent border-b-2 outline-none pb-2 border-red-600" placeholder="Enter your email" type="text">
        <div class="flex xs:flex-row justify-between flex-col text-center"><label
          ...
          <p>Forgot password?</p>
        </div><input class="bg-white/80 text-black w-full py-2 rounded-md duration-300 hover:bg-white/50" type="submit"
          value="Sign In">
      </form>
    </div>
  </section>
