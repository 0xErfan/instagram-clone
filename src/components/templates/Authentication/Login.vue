...
<template>
    ...
    <section class="flex items-center justify-center flex-col size-full container p-4">
                    ...
                    <form @submit.prevent="login" class="mt-6 flex flex-col items-center w-full *:w-full gap-2">
                        <div class="px-2 border border-[#555555] rounded-[2px] bg-secondary-bg relative">
                            <div class="pt-3">
                                ...
                                <div class="absolute top-1/2 -translate-y-1/2 z-0 left-1 w-full text-[12px] transition-all duration-300 text-secondary-text peer-focus:top-2 peer-focus:text-[10px]"
                                    :class="{ 'text-[10px] top-2': formData.payload.trim().length }">
                        ...
                        <div class="px-2 border border-[#555555] rounded-[2px] bg-secondary-bg relative">
                            <div class="pt-3 flex items-center gap-2">
                                ...
                                <div @click="isPasswordHidden = !isPasswordHidden"
                                ...
                                <div class="absolute top-1/2 -translate-y-1/2 z-0 left-1 w-full text-[12px] transition-all duration-300 text-secondary-text peer-focus:top-2 peer-focus:text-[10px]"
                                    :class="{ 'text-[10px] top-2': formData.password.trim().length }">
                                    Password
                                </div>
                            </div>
                        </div>
                        <button :disabled="isLoading" type="submit"
                            class="outline-none my-2 rounded-lg text-center p-1 justify-center text-white bg-btn-primary text-sm"
                            :class="{ 'opacity-70': validation.isFormNotValid }">Log
                            in
                        </button>
        ...
        <div
            class="flex items-center flex-wrap justify-center gap-4 text-secondary-text *:text-nowrap *:w-fit text-[12px] font-normal *:cursor-pointer *:hover:underline">
